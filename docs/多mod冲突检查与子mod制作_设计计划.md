...
> 状态：**已实施（6.90，四阶段交付）** —— 阶段A `d541a75` / 阶段B `c56c84a` / 阶段C `db4eff4` / 阶段D 收尾（含 MCP 工具与文档）。
> 实施记录见 `docs/历史迭代日志.md` 6.90；本文保留为设计基线（实现与计划的差异已在校准：决策复合键、loadable 目录过滤、`resolve_write_path` 收口等）。
