> 状态声明：本文档为历史评估快照，结论对应评估时点的代码状态。进度与缺口以 docs/整合计划.md 批次总表为唯一事实源。
...
- `docs/整合计划.md` 已并入本清单（P0/P1），先修角色→地图→事件/科技，再降级 raw 兜底。

> 进展：批次已按 `docs/整合计划.md` 执行。批 A~E 已标记完成；2026-08-23 已把批次 4/5/6/7/8 从“最小可用/部分”补到完整版，并完成 §0.x 补充项；`ui_gap_probe` 的 event/tech/character/bop 已收敛为 0，state/country_history 按长期豁免说明记录。
