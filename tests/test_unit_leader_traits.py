...
class UnitLeaderTraitsEditorDialogTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _setup(self):
        ...
        _AI_CACHE.clear()
        mod = _mkdtemp("dsh_test_unit_leader_traits_")
        self.addCleanup(shutil.rmtree, mod, ignore_errors=True)
        path = os.path.join(mod, *"common/unit_leader/00_traits.txt".split("/"))
        os.makedirs(os.path.dirname(path), exist_ok=True)
        with open(path, "w", encoding="utf-8") as f:
            f.write("""leader_traits = {
	old_guard = {
		type = land
...
""")
    ...
    def test_parse_and_editor(self):
        from ai_loader import _AI_CACHE, load_unit_leader_traits
        from unit_leader_traits_editor_dialog import UnitLeaderTraitsEditorDialog
        mod, _path = self._setup()
        items = load_unit_leader_traits(mod, "")
        self.assertTrue(len(items) >= 1, items)
        dlg = UnitLeaderTraitsEditorDialog(mod, "")
        dlg.show()
