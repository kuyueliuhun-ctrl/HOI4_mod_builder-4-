...
class DecisionsEditorDialogTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _setup(self):
        ...
        _AI_CACHE.clear()
        mod = _mkdtemp("dsh_test_decisions_")
        self.addCleanup(shutil.rmtree, mod, ignore_errors=True)
        path = os.path.join(mod, *"common/decisions/AFG.txt".split("/"))
...
""")
    ...
    def test_parse_and_editor(self):
        from ai_loader import _AI_CACHE, load_decisions
        from decisions_editor_dialog import DecisionsEditorDialog
        mod, _path = self._setup()
        items = load_decisions(mod, "")
        self.assertTrue(len(items) >= 1, items)
        dlg = DecisionsEditorDialog(mod, "")
        dlg.show()
