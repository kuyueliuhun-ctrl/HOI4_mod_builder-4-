...
class OperationsEditorDialogTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _setup(self):
        ...
        _AI_CACHE.clear()
        mod = _mkdtemp("dsh_test_operations_")
        self.addCleanup(shutil.rmtree, mod, ignore_errors=True)
        path = os.path.join(mod, *"common/operations/00_ops.txt".split("/"))
        os.makedirs(os.path.dirname(path), exist_ok=True)
        with open(path, "w", encoding="utf-8") as f:
            f.write("""operation_test = {
	icon = GFX_operations_x
	days = 35
	network_strength = 30
	operatives = 1
}
""")
    ...
    def test_parse_and_editor(self):
        from ai_loader import _AI_CACHE, load_operations
        from operations_editor_dialog import OperationsEditorDialog
        mod, _path = self._setup()
        items = load_operations(mod, "")
        self.assertTrue(len(items) >= 1, items)
        dlg = OperationsEditorDialog(mod, "")
        dlg.show()
