...
class EquipmentDefinitionsEditorDialogTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _setup(self):
        ...
        _AI_CACHE.clear()
        mod = _mkdtemp("dsh_test_equipment_definitions_")
        self.addCleanup(shutil.rmtree, mod, ignore_errors=True)
        path = os.path.join(mod, *"common/units/equipment/test.txt".split("/"))
...
""")
    ...
    def test_parse_and_editor(self):
        from ai_loader import _AI_CACHE, load_equipment_definitions
        from equipment_definitions_editor_dialog import EquipmentDefinitionsEditorDialog
        mod, _path = self._setup()
        items = load_equipment_definitions(mod, "")
        self.assertTrue(len(items) >= 1, items)
        dlg = EquipmentDefinitionsEditorDialog(mod, "")
        dlg.show()
