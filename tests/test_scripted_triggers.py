"""条件结构体脚本库编辑器测试（B2-P17，共享 RawBlockEditor）。"""
...
class ScriptedTriggersEditorDialogTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _setup(self):
        ...
        _AI_CACHE.clear()
        mod = _mkdtemp("dsh_test_scripted_triggers_")
        self.addCleanup(shutil.rmtree, mod, ignore_errors=True)
        path = os.path.join(mod, *"common/scripted_triggers/00_test.txt".split("/"))
...
""")
    ...
    def test_parse_and_editor(self):
        from ai_loader import _AI_CACHE, load_scripted_triggers
        from scripted_triggers_editor_dialog import ScriptedTriggersEditorDialog
        mod, _path = self._setup()
        items = load_scripted_triggers(mod, "")
        self.assertTrue(len(items) >= 1, items)
        dlg = ScriptedTriggersEditorDialog(mod, "")
        dlg.show()
