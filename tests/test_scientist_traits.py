...
class ScientistTraitsEditorDialogTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _setup(self):
        ...
        _AI_CACHE.clear()
        mod = _mkdtemp("dsh_test_scientist_traits_")
        self.addCleanup(shutil.rmtree, mod, ignore_errors=True)
        path = os.path.join(mod, *"common/scientist_traits/00_traits.txt".split("/"))
        os.makedirs(os.path.dirname(path), exist_ok=True)
        with open(path, "w", encoding="utf-8") as f:
            f.write("""scientist_trait_genius = {
...
""")
    ...
    def test_parse_and_editor(self):
        from ai_loader import _AI_CACHE, load_scientist_traits
        from scientist_traits_editor_dialog import ScientistTraitsEditorDialog
        mod, _path = self._setup()
        items = load_scientist_traits(mod, "")
        self.assertTrue(len(items) >= 1, items)
        dlg = ScientistTraitsEditorDialog(mod, "")
        dlg.show()
