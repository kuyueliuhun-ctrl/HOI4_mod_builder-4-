"""on_actions专用编辑器测试（B3 嵌套实体 + 侧栏 CRUD）。"""
...
class OnActionsEditorDialogTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _setup(self):
        ...
        _AI_CACHE.clear()
        mod = _mkdtemp("dsh_test_on_actions_")
        self.addCleanup(shutil.rmtree, mod, ignore_errors=True)
        path = os.path.join(mod, *"common/on_actions/on.txt".split("/"))
        os.makedirs(os.path.dirname(path), exist_ok=True)
        with open(path, "w", encoding="utf-8") as f:
            f.write("""on_actions = {
	on_startup = {
		effect = { add_political_power = 1 }
	}
}
""")
        return mod, path
    ...
    def test_parse_and_editor(self):
        from ai_loader import _AI_CACHE, load_on_actions
        from on_actions_editor_dialog import OnActionsEditorDialog
        mod, _path = self._setup()
        items = load_on_actions(mod, "")
        self.assertTrue(len(items) >= 1, items)
        dlg = OnActionsEditorDialog(mod, "")
        dlg.show()
