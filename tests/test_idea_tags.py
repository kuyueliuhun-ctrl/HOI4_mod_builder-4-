...
class IdeaTagsEditorDialogTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _setup(self):
        ...
        _AI_CACHE.clear()
        mod = _mkdtemp("dsh_test_idea_tags_")
        self.addCleanup(shutil.rmtree, mod, ignore_errors=True)
        path = os.path.join(mod, *"common/idea_tags/00_idea.txt".split("/"))
        os.makedirs(os.path.dirname(path), exist_ok=True)
        with open(path, "w", encoding="utf-8") as f:
            f.write("""idea_categories = {
	country = {
...
""")
    ...
    def test_parse_and_editor(self):
        from ai_loader import _AI_CACHE, load_idea_tags
        from idea_tags_editor_dialog import IdeaTagsEditorDialog
        mod, _path = self._setup()
        items = load_idea_tags(mod, "")
        self.assertTrue(len(items) >= 1, items)
        dlg = IdeaTagsEditorDialog(mod, "")
        dlg.show()
