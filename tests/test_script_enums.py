"""枚举结构体脚本库编辑器测试（B2-P17，共享 RawBlockEditor）。"""
...
class ScriptEnumsEditorDialogTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _setup(self):
        ...
        _AI_CACHE.clear()
        mod = _mkdtemp("dsh_test_script_enums_")
        self.addCleanup(shutil.rmtree, mod, ignore_errors=True)
        path = os.path.join(mod, *"common/script_enums.txt".split("/"))
        os.makedirs(os.path.dirname(path), exist_ok=True)
        with open(path, "w", encoding="utf-8") as f:
            f.write("""script_enum_test = {
	value_a
	value_b
}
""")
    ...
    def test_parse_and_editor(self):
        from ai_loader import _AI_CACHE, load_script_enums
        from script_enums_editor_dialog import ScriptEnumsEditorDialog
        mod, _path = self._setup()
        items = load_script_enums(mod, "")
        self.assertTrue(len(items) >= 1, items)
        dlg = ScriptEnumsEditorDialog(mod, "")
        dlg.show()
