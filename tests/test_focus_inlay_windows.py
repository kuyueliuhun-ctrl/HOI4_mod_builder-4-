...
class FocusInlayWindowsEditorDialogTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _setup(self):
        ...
        _AI_CACHE.clear()
        mod = _mkdtemp("dsh_test_focus_inlay_windows_")
        self.addCleanup(shutil.rmtree, mod, ignore_errors=True)
        path = os.path.join(mod, *"common/focus_inlay_windows/test.txt".split("/"))
        os.makedirs(os.path.dirname(path), exist_ok=True)
        with open(path, "w", encoding="utf-8") as f:
            f.write("""test_inlay_window = {
	window_name = test_ui_window
	internal = yes
}
""")
        return mod, path
    ...
    def test_parse_and_editor(self):
        from ai_loader import _AI_CACHE, load_focus_inlay_windows
        from focus_inlay_windows_editor_dialog import FocusInlayWindowsEditorDialog
        mod, _path = self._setup()
        items = load_focus_inlay_windows(mod, "")
        self.assertTrue(len(items) >= 1, items)
        dlg = FocusInlayWindowsEditorDialog(mod, "")
        dlg.show()
