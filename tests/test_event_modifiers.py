...
class EventModifiersEditorDialogTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _setup(self):
        ...
        _AI_CACHE.clear()
        mod = _mkdtemp("dsh_test_event_modifiers_")
        self.addCleanup(shutil.rmtree, mod, ignore_errors=True)
        path = os.path.join(mod, *"common/event_modifiers.txt".split("/"))
        os.makedirs(os.path.dirname(path), exist_ok=True)
        with open(path, "w", encoding="utf-8") as f:
            f.write("""TEST_EVENT = {
	icon = GFX_event_modifier
}
""")
        return mod, path

    def test_parse_and_editor(self):
        from ai_loader import _AI_CACHE, load_event_modifiers
        from event_modifiers_editor_dialog import EventModifiersEditorDialog
        mod, _path = self._setup()
        items = load_event_modifiers(mod, "")
        self.assertTrue(len(items) >= 1, items)
        dlg = EventModifiersEditorDialog(mod, "")
        dlg.show()
