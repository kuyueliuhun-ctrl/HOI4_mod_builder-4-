...
- **工具注册表 `src/mcp_tools.py::build_tools(core)`** 返回 **178 个工具**（基础 17 + 域扩展 142 + B3 补 9 + agent 5 + gfx 1 + debug 2 + cwt 2），
...
| **接口规格** | 178 个 MCP 工具逐条、HTTP 端点、dry_run 清单；本文档 §2.4 只有摘要 | 保留 `docs/MCP与接口规格.md` |
...
| 6.87 | 08-31 | venv 切换便携版 | 项目内生成 `portable/win` + `portable/linux` 双平台便携运行时；`launcher.py` 自动按平台选择；`tools/build_portable.py` 支持 `--runtime-only --platform win/linux`；删除原 `.venv`；双端验证使用便携 Python 通过 | docs/便携版打包.md |
