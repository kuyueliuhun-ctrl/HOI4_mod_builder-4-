...
| MCP | `mcp_server.py`（190）、`mcp_tools.py`（651） | stdio 传输；176 个工具注册表（159 + 9 + 5 + 1 + 2，唯一权威来源）；A+B 分类暴露；优先官方 mcp 库，回退内置零依赖实现 | ✅ |
...
- **工具注册表 `src/mcp_tools.py::build_tools(core)`** 返回 **176 个工具**（基础 17 + 域扩展 142 + B3 补 9 + agent 5 + gfx 1 + debug 2），
...
| MCP 工具 | **176** 个（基础 17 + 域扩展 142 + B3 补 9 + agent 5 + gfx 1 + debug 2，`src/mcp_tools.py` 唯一权威） |
...
| **接口规格** | 176 个 MCP 工具逐条、HTTP 端点、dry_run 清单；本文档 §2.4 只有摘要 | 保留 `docs/MCP与接口规格.md` |
...
| MCP 176 工具清单/域分布 | `docs/MCP与接口规格.md` §4 |
...
4 MCP 工具清单（176，域0~域10 + B3 补 9 + agent 5 + gfx 1 + debug 2）→ 5 dry_run 工具清单 → 6 Server 运行 → 6A A+B 分类 → 6B B3 补充 → 7 验证。
