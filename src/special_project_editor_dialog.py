"""特殊计划专用编辑器（B2 P24，simple_entity_tab 动态字段）。

common/special_projects/*.txt 每个顶层块 = 一个特殊计划（标量字段），
...
from ai_loader import load_special_projects, replace_top_block_fields
...
class SpecialProjectEditorDialog(QDialog):
    """特殊计划编辑器。"""

    def __init__(self, projects, mod_path="", hoi4_path="", parent=None,
                 initial_id=None):
        super().__init__(parent)
        self.projects = projects
        self.mod_path = mod_path or ""
        self.hoi4_path = hoi4_path or ""
        self.setWindowTitle("特殊计划编辑器")
        self.resize(1080, 680)

        all_keys = []
        for ent in projects.values():
            for k in ent:
    ...
    def _on_save(self):
        ...
        if not mod_fp:
            QMessageBox.warning(self, "保存失败", "无法定位特殊计划文件")
            return
        ...
        for k, v in fields.items():
            ent[k] = v
        msg = "已保存特殊计划 %s" % ent["id"]
        if copied:
            msg += "\n原版文件已自动复制到 mod"
        QMessageBox.information(self, "已保存", msg)


def open_special_project_editor(file_path="", mod_path="", hoi4_path="",
                                entity_id=None, parent=None):
    """入口：加载并显示特殊计划编辑器（非模态）。"""
    projects = load_special_projects(mod_path, hoi4_path)
    dlg = SpecialProjectEditorDialog(projects, mod_path, hoi4_path,
