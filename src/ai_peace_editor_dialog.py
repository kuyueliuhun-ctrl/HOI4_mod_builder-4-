"""AI 和平策略专用编辑器（B1 P16，simple_entity_tab 动态字段）。

common/ai_peace/*.txt 每个顶层块 = 一个 AI 和平策略（标量字段），
...
from ai_loader import load_ai_peace, replace_top_block_fields
...
class AiPeaceEditorDialog(QDialog):
    """AI 和平策略编辑器。"""

    def __init__(self, peace, mod_path="", hoi4_path="", parent=None,
                 initial_id=None):
        super().__init__(parent)
        self.peace = peace
        self.mod_path = mod_path or ""
        self.hoi4_path = hoi4_path or ""
        self.setWindowTitle("AI 和平策略编辑器")
        self.resize(1080, 680)

        all_keys = []
        for ent in peace.values():
            for k in ent:
    ...
    def _on_save(self):
        ...
        if not mod_fp:
            QMessageBox.warning(self, "保存失败", "无法定位 AI 和平策略文件")
            return
        ...
        for k, v in fields.items():
            ent[k] = v
        msg = "已保存 AI 和平策略 %s" % ent["id"]
        if copied:
            msg += "\n原版文件已自动复制到 mod"
        QMessageBox.information(self, "已保存", msg)


def open_ai_peace_editor(file_path="", mod_path="", hoi4_path="",
                         entity_id=None, parent=None):
    """入口：加载并显示 AI 和平策略编辑器（非模态）。"""
    peace = load_ai_peace(mod_path, hoi4_path)
    dlg = AiPeaceEditorDialog(peace, mod_path, hoi4_path,
