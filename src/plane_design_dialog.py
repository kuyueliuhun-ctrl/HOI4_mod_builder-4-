...
from plane_design import (
    ...
    parse_equipment_variants, _PLANE_VARIANTS_CACHE,
)
...
class PlaneDesignDialog(VariantDesignerBase):
    """飞机设计器主对话框。"""

    KIND = "plane"
    TITLE = "✈ 飞机设计"
    HOST_LABEL = "未选择设计"
    SLOT_COLS = PLANE_SLOT_COLS
    HOSTS_LOADER = load_plane_airframes
    MODULES_LOADER = load_plane_modules
    VARIANTS_LOADER = load_plane_variants
    STATS_FN = plane_design_stats
    TYPE_CN = plane_type_cn_name
    NAME_CN = plane_cn_name
    ...
    VARIANTS_CACHE = _PLANE_VARIANTS_CACHE
    ...
    def _load_data(self):
        self.hosts = load_plane_airframes(self.mod_path, self.hoi4_path)
    ...
    def _rebuild_editor(self):
        ...
        # 布局：上排（主武器/辅助/引擎）+ 下排（特殊槽区）
        top_keys = [k for k in slots
                    if "weapon" in k or k == "engine_type_slot"]
        bottom_keys = [k for k in slots if k not in top_keys]
        row = 1 if not modules else 0
        ...
        def add_zone(title, keys):
            ...
            self.slot_grid.addWidget(
                header, row, 0, 1, PLANE_SLOT_COLS)
            row += 1
            idx = 0
            for slot_key in keys:
                ...
                self.slot_grid.addWidget(
                    card, row + idx // PLANE_SLOT_COLS,
                    idx % PLANE_SLOT_COLS)
                self._slot_buttons[slot_key] = btn
                idx += 1
            row += (idx + PLANE_SLOT_COLS - 1) // PLANE_SLOT_COLS

        add_zone("固定装备区（主武器/辅助/引擎）", top_keys)
        add_zone("特殊槽区（下排）", bottom_keys)
