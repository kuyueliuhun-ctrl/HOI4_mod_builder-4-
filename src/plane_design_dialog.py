...
class PlaneDesignDialog(QDialog):
    ...
    def _add_design(self):
        ...
        keys = [k for k in self.airframes
                if not self.airframes[k].get("is_archetype")]
        if not keys:
            QMessageBox.warning(self, "无法新建", "未找到可用机体。")
            return
        items = []
        for k in sorted(keys, key=lambda x: (self.airframes[x].get("year") or 0, x)):
            cn = plane_type_cn_name(k)
            yr = self.airframes[k].get("year")
            items.append("%s  %s  [%s]" % (cn, yr if yr else "", k))
        choice, ok = QInputDialog.getItem(
            self, "选择机体", "机体（中文名 + 年份 + 键）:", items, 0, False)
        if not ok:
            return
        key = choice.rsplit("[", 1)[-1].rstrip("]")
        if key not in self.airframes:
            return
