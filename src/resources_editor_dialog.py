"""资源专用编辑器（B3/P34，simple_entity_tab 动态字段）。

common/resources/*.txt 每个顶层块 = 一个资源定义（标量字段），
...
from ai_loader import load_resources2, replace_top_block_fields
...
class ResourcesEditorDialog(QDialog):
    """资源编辑器。"""

    def __init__(self, resources, mod_path="", hoi4_path="", parent=None,
                 initial_id=None):
        super().__init__(parent)
        self.resources = resources
        self.mod_path = mod_path or ""
        self.hoi4_path = hoi4_path or ""
        self.setWindowTitle("资源编辑器")
        self.resize(1080, 680)

        all_keys = []
        for ent in resources.values():
            for k in ent:
        ...
        entities = []
        for rid, ent in resources.items():
            e = {"id": rid, "name": rid}
            for k in all_keys:
                e[k] = ent.get(k, "")
            e["rel"] = ent.get("rel", "")
    ...
    def _on_save(self):
        ...
        if not mod_fp:
            QMessageBox.warning(self, "保存失败", "无法定位资源文件")
            return
...
def open_resources_editor(file_path="", mod_path="", hoi4_path="",
                          entity_id=None, parent=None):
    """入口：加载并显示资源编辑器（非模态）。"""
    resources = load_resources2(mod_path, hoi4_path)
    dlg = ResourcesEditorDialog(resources, mod_path, hoi4_path,
