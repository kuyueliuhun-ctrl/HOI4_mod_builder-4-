"""AI 人格专用编辑器（B1 P16，simple_entity_tab 动态字段）。

common/ai_personalities/*.txt 每个顶层块 = 一个 AI 人格，其标量字段
...
from ai_loader import load_ai_personalities, replace_top_block_fields
...
class AiPersonalitiesEditorDialog(QDialog):
    """AI 人格编辑器。"""

    def __init__(self, personalities, mod_path="", hoi4_path="", parent=None,
                 initial_id=None):
        super().__init__(parent)
        self.personalities = personalities
        self.mod_path = mod_path or ""
        self.hoi4_path = hoi4_path or ""
        self.setWindowTitle("AI 人格编辑器")
        self.resize(1080, 680)

        all_keys = []
        for ent in personalities.values():
            for k in ent:
        ...
        entities = []
        for pid, ent in personalities.items():
            e = {"id": pid, "name": pid}
            for k in all_keys:
                e[k] = ent.get(k, "")
            e["rel"] = ent.get("rel", "")
    ...
    def _on_save(self):
        ...
        if not mod_fp:
            QMessageBox.warning(self, "保存失败", "无法定位 AI 人格文件")
            return
        ...
        for k, v in fields.items():
            ent[k] = v
        msg = "已保存 AI 人格 %s" % ent["id"]
        if copied:
            msg += "\n原版文件已自动复制到 mod"
        QMessageBox.information(self, "已保存", msg)


def open_ai_personalities_editor(file_path="", mod_path="", hoi4_path="",
                                 entity_id=None, parent=None):
    """入口：加载并显示 AI 人格编辑器（非模态）。"""
    personalities = load_ai_personalities(mod_path, hoi4_path)
    dlg = AiPersonalitiesEditorDialog(personalities, mod_path, hoi4_path,
