...
def _playset_tools(core):
    return [
        ...
        _tool("submod_status", "子mod模式状态：当前层栈（各层名称/路径/类型）与 settings 持久化字段",
              _obj({}),
              lambda args: core.submod_status(args)),
        _tool("submod_create", "创建子 mod（dependencies=底层 name 的真实可玩 mod）并可选激活；激活后读写路由经层栈：读=子mod→底层→原版合并视图，写=恒落子mod（copy_up 自动）",
              _obj({
                  "submod_name": _str("子 mod 名称"),
                  "folder_name": _str("文件夹名（缺省按名称生成）"),
                  "version": _str("supported_version（默认 1.19.*）"),
                  "tags": _arr(_str("标签"), "标签列表"),
                  "base_names": _arr(_str("底层 mod descriptor name"), "依附的底层 mod（缺省 = 播放集全部）"),
                  "base_paths": _arr(_str("底层 mod 内容目录"), "显式底层路径（优先于 base_names）"),
                  "playset_id": _str("播放集 id（缺省 = dlc_load 最近启动集）"),
                  "user_dir": _str("HOI4 用户文档目录（缺省读 settings.json 推断）"),
                  "read_all": _bool("读取范围 = 整个播放集（默认 true；false 则仅底层勾选）"),
                  "mod_folder_path": _str("子 mod 创建目录（缺省读 settings.json）"),
                  "mod_file_path": _str(".mod 文件目录（缺省 = mod_folder_path）"),
                  "activate": _bool("创建后是否激活子 mod 模式（默认 true）"),
                  "persist": _bool("激活时是否写 settings.json（默认 true）"),
              }, ["submod_name"]),
              lambda args: core.submod_create(args)),
        _tool("submod_activate", "激活已有子 mod（进程内层栈；persist=true 时写 settings.json 供 GUI 重启恢复）",
              _obj({
                  "submod_path": _str("子 mod 内容目录"),
                  "base_paths": _arr(_str("底层 mod 内容目录"), "底层读取层（缺省读 settings.json 的 submod_bases）"),
                  "submod_name": _str("显示名（缺省取目录名）"),
                  "persist": _bool("是否写 settings.json（默认 true）"),
              }, ["submod_path"]),
              lambda args: core.submod_activate(args)),
        _tool("submod_exit", "退出子 mod 模式（清进程内层栈；persist=true 时同步 settings.json）",
              _obj({"persist": _bool("是否写 settings.json（默认 false，仅清本进程）")}),
              lambda args: core.submod_exit(args)),
    ]
...
def _domain10_tools(core):
    return [
        ...
        _tool("copy_country_files", "复制原版国家文件到 mod（默认 dry_run）",
              _obj({"tag": _str("国家 tag"), "dirs": _arr(_str(), "mod 内相对目录列表"),
...
_CATEGORY_TOOLS = {
    ...
    "project": [
        "submod_status", "submod_create", "submod_activate", "submod_exit",
