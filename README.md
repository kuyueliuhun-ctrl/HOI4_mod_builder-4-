...
| 地图编辑器 | 三栏布局（建筑类型 / 画布 / 地块信息），框选划分 strategicregions / supplyareas / states；**数据层色阶**（VP/资源总量/补给区/铁路/河流覆盖层）；**导出整图**（按图层合成完整世界地图 PNG） |
...
- **外部接口**：HTTP API（仅绑定 127.0.0.1 + Bearer token）+ MCP 服务器（**178 个工具**）
...
| [`docs/MCP与接口规格.md`](./docs/MCP与接口规格.md) | 178 个 MCP 工具、HTTP 端点、ApiCore 约定 |
